...
func handle(c net.Conn) {
	...
	for {
		...
		if dataFromCli == "exit" {
		...
		} else if strings.HasPrefix(dataFromCli, "cat ") {
			...
			for i, letter := range dataFromCli {
				...
				if tmpLetter == "cat " {
					tmpFile = dataFromCli[i+1:]
					break
				}

			}

			// tmpFile = file to read
			...
			if tmpLgc == true {
				...
				tmpFile = crrPth + "\\" + tmpFile

				openFile, err := os.Open(tmpFile)
				if err != nil {
				...
				if err != nil {
					fmt.Println("[ERR] Read the file:", err)
				}
				...
				//
				// NOTE: Maybe adding a fmt.Println("[INFO] Done")
				//

			} else {

				dataToCli = "\n[ERR]" + tmpFile + "does not exist.\n"

				fmt.Printf("[ERR] " + tmpFile + " does not exist.\n")
				c.Write([]byte(dataToCli))
